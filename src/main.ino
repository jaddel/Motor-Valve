...
//bool valve_config.manual_mode = false;
uint16_t runtime = 0;
...
String ssid = "motor-valve" ;//+ String(ESP_getChipId(),HEX)[0] + String(ESP_getChipId(),HEX)[1]; //"ESP_" + String(ESP_getChipId(), HEX);
String password;
...
typedef struct
{
  ...
  uint16_t limit;   //in C
  bool manual_mode;  //Mode manual or automatic (1=manual)
...
void handleWWWApp(AsyncWebServerRequest *request) {
  ...
  for (uint8_t i = 0; i < request->args(); i++) {
    ...
    {
      valve_config.manual_mode = true;
      news = true;
    }
    if (request->argName(i) == "auto") {
      valve_config.manual_mode = false;
      news = true;
      runtime = 0;
    }
    ...
    if (request->argName(i) == "manboiler") {
      ...
      q_solar = false;
      runtime = 0;
      Serial.println("Man Boiler from Web");
    }
    ...
    if (request->argName(i) == "mansolar") {
      ...
      q_solar = true;
      runtime = 0;
      Serial.println("Man Solar from Web");
    }
    ...
    if (request->argName(i) == "manstop") {
      ...
      q_solar = false;
      runtime = 0;
      Serial.println("Man Stop from Web");
    }
  }

  if (news)
  {
    Serial.println("save config data now");
    saveConfigData();
  ...
  page += "<td>";
  page += (valve_config.manual_mode) ? "Manual Mode" : "Automatic Mode";
  page += "</td>";
  ...
  page += (valve_config.manual_mode) ? "Automatic Mode" : "Manual Mode";
  page += "\" ";
  page += "name=\"";
  page += (valve_config.manual_mode) ? "auto" : "manual";
  page += "\" class=\"btn\"> ";

  if (valve_config.manual_mode) {
    page += "<input type=\"submit\" value=\"manual Boiler\" name=\"manboiler\" class=\"btn\">";
    page += "<input type=\"submit\" value=\"manual Solar\" name=\"mansolar\" class=\"btn\">";
    page += "<input type=\"submit\" value=\"manual Stop\" name=\"manstop\" class=\"btn\">";
  }
  ...
  Serial.println("preparation html done");
  ...
  Serial.println("resonse via http done");
}
...
void setup() {
  ...
  if (initialConfig) {
    ...
    memset(&valve_config, 0, sizeof(valve_config));
    valve_config.limit = 30;
    valve_config.offset = 2;
    valve_config.time = 30000;
    valve_config.manual_mode = true;
    saveConfigData();
...
void loop() {
  ...
  Serial.println("digital write done");

  //Button management
    if(button_time > 1500)
    { //menue_select = 5; //only for debug
    Serial.println("exe button mgnt");
    if (menue_select == 1)   //stopp all
    {
      valve_config.manual_mode = 0;
      menue_select = 0;
    } else if (menue_select == 2) {
      valve_config.manual_mode = 1;
      q_boiler = 1;
      q_solar = 0;
      menue_select = 0;
  ...
  if (menue_select > 0) {
    ...
    if (menue_select <= 4) {
      if (menue_select == 1) display.write(0x10);
      else display.write(" ");
      display.write("1: Automatic mode\n");
      if (menue_select == 2) display.write(0x10);
      else display.write(" ");
      display.write("2: manual Boiler\n");
      if (menue_select == 3) display.write(0x10);
      else display.write(" ");
      display.write("3: manual Solar\n");
      if (menue_select == 4) display.write(0x10);
      else display.write(" ");
      display.write("4: Exit\n");
    } else {
      if (menue_select == 5) display.write(0x10);
      else display.write(" ");
      display.write("5: Wifi config\n");
      if (menue_select == 6) display.write(0x10);
      else display.write(" ");
      display.write("6: Reboot\n");
      if (menue_select == 7) display.write(0x10);
      else display.write(" ");
      display.write("7: \n");
      if (menue_select == 8) display.write(0x10);
      else display.write(" ");
